...
export class NPCInteractApp extends Application {
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      title: game.i18n.localize("KULT.NPCAuto.Interact"),
  ...
  getData() {
    const player =
      game.user?.character ?? game.actors.filter((a) => a.hasPlayerOwner)[0];
    const moneyPaths = getCurrencyPaths();
    const playerCurrency = player
      ? foundry.utils.getProperty(player, moneyPaths.player) ?? 0
      : 0;
    const npcCurrency =
      foundry.utils.getProperty(this.actor, moneyPaths.npc) ?? 0;

    const inventory = (this.flags.shop?.inventory ?? []).filter(
      (i) => (i.qty ?? 0) > 0
    );
    const rawNode =
      this.flags.dialogue?.nodes?.find((n) => n.id === this.currentNodeId) ??
      null;
    ...
    const ctx = {
      ...
      actorPC: player,
      user: game.user,
      flagsRoot: this.flags,
    };
    const node = rawNode
      ? { ...rawNode, options: getAvailableOptions(rawNode, ctx) }
      : null;

    const viewInventory = inventory.map((it) => {
      const priceView = calcPriceDisplay(it.price, {
        markupRate: moneyPaths.markupRate,
      });
      return { ...it, priceView };
    });
  ...
  activateListeners(html) {
    ...
    if (game.user.isGM) {
      html.find("[data-action='open-editor']").on("click", async () => {
        const { DialogueEditorApp } = await import("./DialogueEditorApp.js");
        new DialogueEditorApp(this.actor).render(true);
      });
    }
  }
  ...
  async _buy(ev) {
    try {
      ...
      const priceView = calcPriceDisplay(item.price, {
        markupRate: moneyPaths.markupRate,
      });
      ...
      if (!player)
        return ui.notifications.warn(
          game.i18n.localize("KULT.NPCAuto.WarnNoPC")
        );
      ...
      ui.notifications.info(
        game.i18n.format("KULT.NPCAuto.BuyOK", {
          qty,
          name: item.name,
          price: priceFinal,
        })
      ...
      ui.notifications.error(
        e.message || game.i18n.localize("KULT.NPCAuto.ErrBuy")
      );
    }
  }
  ...
  _openSell() {
    const player =
      game.user?.character ?? game.actors.filter((a) => a.hasPlayerOwner)[0];
    if (!player)
      return ui.notifications.warn(game.i18n.localize("KULT.NPCAuto.WarnNoPC"));
