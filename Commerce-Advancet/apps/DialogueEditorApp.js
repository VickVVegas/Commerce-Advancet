...
export class DialogueEditorApp extends Application {
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      title: game.i18n.localize("KULT.NPCAuto.Editor"),
  ...
  activateListeners(html) {
    ...
    html.find("[data-action='add-node']").on("click", async () => {
      ...
      nodes.push({
        id,
        text: game.i18n.localize("KULT.NPCAuto.NewNodeText"),
        options: [],
      });
    ...
    html.find("[data-action='save-node']").on("click", async (ev) => {
      ...
      await setNPCFlags(this.actor, { "dialogue.nodes": nodes });
      this.flags = getNPCFlags(this.actor);
      ui.notifications.info(game.i18n.localize("KULT.NPCAuto.NodeSaved"));
    });
    ...
    html.find("[data-action='add-option']").on("click", async () => {
      ...
      nodes[idx].options.push({
        label: game.i18n.localize("KULT.NPCAuto.NewOption"),
        next: "",
        conditions: null,
        visibleForUsers: [],
      });
    ...
    html.find("[data-action='save-option']").on("click", async (ev) => {
      ...
      try {
        ...
        return ui.notifications.error(
          game.i18n.localize("KULT.NPCAuto.ErrCondJSON")
        );
      }
      ...
      await setNPCFlags(this.actor, { "dialogue.nodes": nodes });
      this.flags = getNPCFlags(this.actor);
      ui.notifications.info(game.i18n.localize("KULT.NPCAuto.OptionSaved"));
    });
  }
}
